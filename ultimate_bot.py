...
class MyClient(discord.Client):
    ...
    async def on_message(self,message):
        command_list = {
                        "Chatear con el Bot": "!hello",
                        "Borrar mensaje del Bot": "!deletme!",
                        "Auto Editar el mensaje del Bot": "!editme",
                        "Generar contraseña aleatoria": "!password",
                        "Hacer un coin flip": "!coin"
                        }
        if message.author == self.user.id:
            return
        if message.content.startswith("!help"):
            await message.channel.send(f"Aquí está la lista de comandos: {command_list}")  

intents = discord.Intents.default()
intents.message_content = True

client = MyClient(intents=intents)
client.run('INTRODUCE EL TOKEN DE TU BOT AQUÍ')
